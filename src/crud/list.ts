...
import { GraphQLList, GraphQLType, GraphQLObjectType, GraphQLInt, GraphQLResolveInfo, GraphQLString, GraphQLInputObjectType, GraphQLScalarType } from 'graphql';

import joi from 'joi';
import { Model, DocumentQuery, Document } from 'mongoose';
import { apiWrapper, Options, LocalRequest } from './apiWrapper';
import {parseLiteral} from "../utils/parser"
...
function list<T extends Document, Q, A extends { [key: string]: { type: GraphQLType } }, C>(
  ...
  async function pre(a: { page: number; perPage: number; order: string; sort: number } & { [K in keyof A]: any }, req: LocalRequest) {
    initialArgs = a;
    const args = { ...a };
  ...
  async function getList(
    ...
    const { perPage, page, sort: sortArgs, order, filter, ...rest } = args;
    let count = 0;

    let qfilter : any = {}
    if (filter){
      let {fieldName, operation, values, value} = filter
  
      qfilter  = operation? {[fieldName] : {[`$${operation}`]: values? values : value }} : {[fieldName]:value}      
    }

    qfilter = {...qfilter, ...rest}
    ...
    const result =
      perPage > 0
        ? model
            ...
            .skip(perPage * (page - 1))
            .limit(perPage)
        : model.find(qfilter as any).sort(sort);

    return {
      count,
      data: options.post ? options.post({ result: result as any, args: args as any, request: req, initialArgs, info }) : result,
  ...
  return apiWrapper(
    ...
    {
      ...options,
      validateSchema: options.validateSchema ? { ...listValidationSchema, ...options.validateSchema } : listValidationSchema,
      pre,
      post: undefined,
    } as any
  );
